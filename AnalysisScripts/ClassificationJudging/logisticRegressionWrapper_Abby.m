...
for i = 1:sim_number
    ...
    %% Simulate a subject and create features table
        ...
        [thresholds(:,i)]= simulateSubject(mu, sigma, withSR, N, initial_stim, underlyingMuRatios);
    ...
    %% Ratio of best threshold to sham, 4th column of 'features'
        features(i,4) = min(thresholds(:,i))/(sham(i)+thresholds(end,i));
    ...
    %% Avg of best and 2 neighboring, 5th column of 'features'
        ...
        features(i,5) = mean(minthresholds)/(sham(i)+thresholds(end,i)); 
    %% Clarify if the subject has SR or no SR
        ...
        features(i,7) = min(thresholds(:,i))/max(thresholds(:,i))/sigma;
