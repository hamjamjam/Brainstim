...
function wrapper_code_Visual(app)
...
while (k < trial_no )   %This loop is for the initial stimuli until the first mistake
    ...
    while (n_down_PEST_cnt < n_down_PEST && n_up_PEST_cnt < n_up_PEST )
        ...
        elseif trial_stim(n)>1
            % value cannot be greater than 1
            trial_stim_run(n) = 1;
        else
            %just use calculated stim value
        ...
        elseif n==1
            ...
            [response] = hardwarefunc_Visual(trial_stim_run(n),screens,screenNumber,white,grey,black,inc,window,windowRect,app);
            l_or_r(n) = response;
        else
            %not complete
            app.StimDisplayLabel.Text=sprintf('%g',abs(trial_stim_run(n)));
