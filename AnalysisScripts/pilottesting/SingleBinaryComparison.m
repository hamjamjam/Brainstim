...
switch TestType
    case 1 % define visual folder information
        mu_guess = 0.2; sigma_guess = 0.05; N=50;
        if SRType == 1
            parseFold1=['BSRA1_' num2str(subID) '_Visual_0mA_'];
            parseFold2=['dB.xls'];
        elseif SRType == 2
            parseFold1=['BSRA1_' num2str(subID) '_Visual_'];
            parseFold2=['mA_0dB.xls'];
        else
        end
    case 2 % define auditory folder information
        mu_guess = 8; sigma_guess = 3; N=100;
        if SRType == 1
            parseFold1=['BSRA1_' num2str(subID) '_Auditory_0mA_'];
            parseFold2=['dB.xls'];
        elseif SRType == 2
            parseFold1=['BSRA1_' num2str(subID) '_Auditory_'];
            parseFold2=['mA_0dB.xls'];
        else
        end
    case 3 % define tactile folder information
        mu_guess = 0.3; sigma_guess = 0.1; N=50;
        if SRType == 1
            parseFold1=['BSRA1_' num2str(subID) '_Tactile_0mA_'];
            parseFold2=['dB.xls'];
        elseif SRType == 2
            parseFold1=['BSRA1_' num2str(subID) '_Tactile_'];
            parseFold2=['mA_0dB.xls'];
        else
        end
    case 4
        mu_guess = 0; sigma_guess = 0; N=100;
        if SRType == 1
            parseFold1=['BSRA1_' num2str(subID) '_Vestibular_0mA_'];
            parseFold2=['dB.xls'];
        elseif SRType == 2
            parseFold1=['BSRA1_' num2str(subID) '_Vestibular_'];
            parseFold2=['mA_0dB.xls'];
        else
        end
end
...
switch TestType
    ...
    case 3
        textscale = -.1;
        if SRType == 1
            tickMarks=[20 30:5:80]; xlimit=[17.5 82.5]; ylimit=[-.15 .4]; Toffset=1;
            xticklabs={'No Noise' '30' '35' '40' '45' '50' '55' '60' '65' '70' '75' '80'};
        elseif SRType == 2
            tickMarks=[-.1 .1:.1:1]; xlimit=[-.3 1.2]; ylimit=[-.15 .6]; Toffset=.025;
            xticklabs={'No Noise' '.1' '.2' '.3' '.4' '.5' '.6' '.7' '.8' '.9' '1'};
